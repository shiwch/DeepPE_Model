...
class Net(nn.Module):
    def __init__(self, hidden_size, hidden_dim, learning_rate):
        ...
        self.batch_size = 256
        self.epochs = 100
        self.net_type = "bi-HAC-att-8-upper"
        self.dim_weight = 5
        self.input_data = {}
        self.output_data = {}
        self.datas = {}
...
for test_num in [2, 19, 21, 22, 31, 37, 38, 49, 50, 65, 66, 68, 72, 80, 82, 102]:

    model = Net(2, 8, 0.001).cuda()
    model.change(test_num)
    ...
    pred_df.to_csv("./migration_training/"+model.net_type+"/"+"_pred_"+"%03d"%test_num+".csv", index=False)
    orig_df.to_csv("./migration_training/"+model.net_type+"/"+"_orig_"+"%03d"%test_num+".csv", index=False)
