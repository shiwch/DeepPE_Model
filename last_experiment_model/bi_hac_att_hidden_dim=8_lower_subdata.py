...
class Net(nn.Module):
    def __init__(self, hidden_size, hidden_dim, learning_rate):
        ...
        self.batch_size = 256
        self.epochs = 100
        self.net_type = "bi-HAC-att-8-lower-subdata"
        self.dim_weight = 5
        self.input_data = {}
        self.output_data = {}
        self.datas = {}
    ...
    def merge_data(self):
        train_data = torch.cat([self.train_x, self.train_y], dim = 1)
        for i in [2, 19, 21, 22, 31, 37, 49, 50, 65, 66, 68, 72, 80, 82, 102]:
        ...
        train_data = train_data[index[0:train_data.shape[0]//16],:]
        self.train_x = train_data[:,range(self.train_x.shape[1])]
        self.train_y = train_data[:,range(self.train_x.shape[1], train_data.shape[1])]
        print(self.train_x.shape)
...
print("train_time: " + str(end - start))
train_time = train_time + end - start
    
for i in [2, 19, 21, 22, 31, 37, 38, 49, 50, 65, 66, 68, 72, 80, 82, 102]:

    start = time.time()
    y_hat = model(model.test_xx[i].cuda()).cpu()
    end = time.time()
    print("test_train_time "+str(i)+"th : " + str(end - start))
    test_pred = numpy.concatenate((model.test_xx[i].numpy(), y_hat.detach().numpy()), axis = 1)
    test_orig = numpy.concatenate((model.test_xx[i].numpy(), model.test_yy[i].numpy()), axis = 1)
    #print(y_hat.detach())
    #print("***************************")
    #print(y_hat)
    ...
    pred_df.to_csv("./migration_training/"+model.net_type+"/"+"_pred_"+"%03d"%i+".csv", index=False)
    orig_df.to_csv("./migration_training/"+model.net_type+"/"+"_orig_"+"%03d"%i+".csv", index=False)
