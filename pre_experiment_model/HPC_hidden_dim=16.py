...
class Net(nn.Module):
    def __init__(self, hidden_size, hidden_dim, learning_rate):
        ...
        self.outputs_hpc = nn.ModuleList()
        for i in range(self.block_levels):
            ...
            self.outputs_hpc = nn.Linear(self.hidden_dim, self.output_dim)
    ...
    def forward(self, input_data):
        ...
        for i in range(self.block_levels):
            out = self.inputs_hpc[i](prev)
            for j in range(len(self.blocks[i])):
                out = self.blocks[i][j](out)
            out = self.outputs_hpc(out)

            prev = torch.cat([input_data, out], dim = 1)
