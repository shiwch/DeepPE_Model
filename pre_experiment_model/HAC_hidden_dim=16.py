...
class Net(nn.Module):
    def __init__(self, hidden_size, hidden_dim, learning_rate):
        ...
        self.inputs_hac = nn.ModuleList()
        self.outputs_hac = nn.ModuleList()
        ...
        for i in range(self.block_levels):
            ...
            #self.inputs_hpc.append(nn.Linear(self.input_dim + ((i > 0) + 0) * self.output_dim, self.hidden_dim))
            self.inputs_hac.append(nn.Linear(self.input_dim + i * self.output_dim, self.hidden_dim))
            self.blocks.append(self.block)
            self.outputs_hac.append(nn.Linear(self.hidden_dim, self.output_dim))
    ...
    def forward(self, input_data):
        ...
        for i in range(self.block_levels):
            ...
            out = self.inputs_hac[i](prev)
            for j in range(len(self.blocks[i])):
                out = self.blocks[i][j](out)
            out = self.outputs_hac[i](out)
            prev = torch.cat([input_data, total_out, out], dim = 1)
