...
class Net(nn.Module):
    def __init__(self, hidden_size, hidden_dim, learning_rate):
        ...
        self.inputs_fnn = nn.ModuleList()
        self.outputs_fnn = nn.ModuleList()
        ...
        for i in range(self.block_levels):
            ...
            self.inputs_fnn.append(nn.Linear(self.input_dim + ((i > 0) + 0) * (self.output_dim - self.input_dim), self.hidden_dim))
            # self.inputs_hac.append(nn.Linear(self.input_dim + i * self.output_dim, self.hidden_dim))
            self.blocks.append(self.block)
            self.outputs_fnn.append(nn.Linear(self.hidden_dim, self.output_dim))
        ...
        self.output_fnn = nn.Linear(self.output_dim, self.output_dim * self.block_levels)

    def forward(self, input_data):
        
        prev = input_data
        total_out = torch.Tensor().cuda()
        out = torch.Tensor().cuda()
                
        for i in range(self.block_levels):
            out = self.inputs_fnn[i](prev)
            for j in range(len(self.blocks[i])):
                out = self.blocks[i][j](out)
            out = self.outputs_fnn[i](out)
            prev = out
            # out_list.append(out)
        total_out = self.output_fnn(prev)
